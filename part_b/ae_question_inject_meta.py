...
def main():
    ...
    epoch_list = [10, 15, 20, 25]  # 3, 5, 10, 15
    test_accuracy_list = []
    ...
    for k in k_list:
        for lr in lr_list:
            for num_epoch in epoch_list:
                model = AutoEncoder(train_matrix.shape[0], k, k_meta + 1)
                train(model, lr, lamb, train_matrix, zero_train_matrix,
                      valid_data, num_epoch, betas=betas, metas=metadata)
                test_accuracy = evaluate(model, zero_train_matrix, test_data, betas=betas, metas=metadata)
