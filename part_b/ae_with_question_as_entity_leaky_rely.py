...
class AutoEncoder(nn.Module):
    def __init__(self, num_students, k=100, extra_latent_dim=0):
        ...
        nn.init.kaiming_normal_(self.f.weight)
    ...
    def forward(self, inputs, beta=None):
        ...
        layer_one = leaky_relu(self.f(inputs))
        layer_two = self.g(layer_one)
        layer_three = leaky_relu(self.h(layer_two))
        layer_four = self.i(layer_three)
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
        return layer_four
