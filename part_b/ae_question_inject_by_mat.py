...
class AutoEncoder(nn.Module):
    def __init__(self, num_students, k=100, extra_latent_dim=0):
        ...
        self.last = nn.Linear(num_students * 2, num_students)
    ...
    def forward(self, inputs, beta=None, theta=None):
        ...
        raw_pred_append = torch.cat((raw_pred, theta.unsqueeze(0)), dim=-1)
        theta_inject_pred = self.last(raw_pred_append)
    ...
    for epoch in range(0, num_epoch):
        ...
        for question_id in range(num_question):
            ...
            if betas is not None:
                ...
                output = model(inputs, beta, thetas)
            else:
                output = model(inputs)
...
def evaluate(model, train_data, valid_data, betas, thetas):
    ...
    for i, q in enumerate(valid_data["question_id"]):
        ...
        if betas is not None:
            output = model(inputs, betas[q], thetas)
        else:
            output = model(inputs)
...
def main():
    ...
    k_list = [10]  # 10, 50, 100, 200
    lr_list = [0.1]  # 0.001, 0.01, 0.1, 1
    epoch_list = [20, 25, 30]  
    test_accuracy_list = []
